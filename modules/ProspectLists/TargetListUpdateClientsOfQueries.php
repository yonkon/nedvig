...
if (count($uids)>0){
    ...
    $focus = new sphr_Client();
    
    $query = 'SELECT DISTINCT 	sphr_clien735b_client_ida AS id FROM sphr_clientosib_queries_c WHERE sphr_clien6a3fqueries_idb  IN ("'.implode('","',$uids).'") AND deleted = 0';
    $result = $GLOBALS['db']->query($query,true);
    $uids = array();
