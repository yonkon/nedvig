...
class ProspectList extends SugarBean {
	...
	function create_export_members_query($record_id)
	{
		...
		$accounts_query = "SELECT a.id AS id, 'Accounts' AS related_type, a.name AS \"name\", '' AS first_name, '' AS last_name,
				a.billing_address_street AS primary_address_street, ac.city_c AS primary_address_city, ac.region_c AS primary_address_state, a.billing_address_postalcode AS primary_address_postalcode, ac.country_c AS primary_address_country,
				'' AS account_name,
				ea.email_address AS email_address, ea.invalid_email AS invalid_email, ea.opt_out AS opt_out, ea.deleted AS ea_deleted, ear.deleted AS ear_deleted, ear.primary_address AS primary_address,
				0 AS do_not_call, a.phone_fax as phone_fax, '' AS phone_other, '' AS phone_home, a.phone_alternate AS phone_mobile, a.phone_office AS phone_office
				FROM prospect_lists_prospects plp
				...
				WHERE plp.prospect_list_id = $record_id  AND plp.deleted=0
				AND a.deleted=0";
                $sphr_Client_query = "SELECT c.id AS id, 'sphr_Client' AS related_type, '' AS name, c.first_name AS first_name, c.last_name AS last_name,
				cc.current_address_c AS primary_address_street, cc.city_c AS primary_address_city, cc.region_c AS primary_address_state,  '' AS primary_address_postalcode, cc.country_c AS primary_address_country,
                                a.name AS account_name,
				ea.email_address AS email_address, ea.invalid_email AS invalid_email, ea.opt_out AS opt_out, ea.deleted AS ea_deleted, ear.deleted AS ear_deleted, ear.primary_address AS primary_address,
				c.do_not_call AS do_not_call, '' AS phone_fax, cc.phone_mobile_2_c AS phone_other, c.phone_home AS phone_home, c.phone_mobile AS phone_mobile, c.phone_work AS phone_work
				FROM  prospect_lists_prospects plp
				INNER JOIN sphr_client c ON plp.related_id=c.id
                                LEFT JOIN sphr_client_cstm cc ON cc.id_c = c.id
				LEFT JOIN email_addr_bean_rel ear ON  ear.bean_id=c.id 
				LEFT JOIN email_addresses ea ON ear.email_address_id=ea.id
                                LEFT JOIN accounts_sphr_client_c ac ON ac.accounts_sde91_client_idb = c.id AND ac.deleted=0
                                LEFT JOIN accounts a ON ac.accounts_s160fccounts_ida = a.id AND a.deleted = 0
				WHERE plp.prospect_list_id = $record_id  AND plp.deleted=0
				AND c.deleted=0 ";
                
		$order_by = "ORDER BY related_type, id, primary_address DESC";
		//$query = "$leads_query UNION ALL $users_query UNION ALL $contacts_query UNION ALL $prospects_query UNION ALL $accounts_query UNION ALL $sphr_Client_query $order_by";
                $query = "$users_query UNION ALL $prospects_query UNION ALL $accounts_query UNION ALL $sphr_Client_query $order_by";
		return $query;
	}
