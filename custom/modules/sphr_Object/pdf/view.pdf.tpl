...
<head></head>
<body>
{{include file="custom/modules/sphr_Object/pdf/style.tpl"}}
...
{{/if}}
<br>
{foreach from=$ar_photos key=old_photo_id item=old_photo_value name=fore_photo}
	{if $smarty.foreach.fore_photo.first}
		{$MOD.LBL_PHOTOS}<br>
	{/if}
	<img src="{$old_photo_id}"/><br>
	<span>{$old_photo_value}</span><br><br>
{/foreach}
<br>
{foreach from=$ar_layouts key=old_layout_id item=old_layout_value name=fore_layout}
	{if $smarty.foreach.fore_layout.first}
		{$MOD.LBL_LAYOUTS}<br>
	{/if}
	<img src="{$old_layout_id}"/><br>
	<span>{$old_layout_value}</span><br><br>
{/foreach}
