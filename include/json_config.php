...
class json_config {
	...
	function meeting_retrieve($module, $record) {
		...
		require_once($beanFiles[$beanList[$module]]);
		$focus = new $beanList[$module];
		//print("beanFiles<pre>");	print_r($beanFiles);
		//print("beanList");		print_r($beanList);print("</pre>");
		...
		if($module == 'sphr_Show_object') {
			$users = $focus->get_sphr_show_object_users();
		} 
	...
	function populateBean(&$focus) {
		...
		//print ("all_fields<pre>"); print_r  ($all_fields);print("</pre>");
		// MEETING SPECIFIC
