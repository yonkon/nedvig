...
//-----------------------------------20.07.14       
		if(strtolower($vardef['name']) == 'province_select_c'){
			require_once('custom/include/language/handler.php');
			$result = getProvince($string);
			if($result != false){
				$string=$result;
			}
		}

		if(strtolower($vardef['name']) == 'coast_select_c'){
			require_once ('custom/include/language/handler.php');
			$result = getCoast($string);
			if($result != false){
				$string=$result;
			}
		}
