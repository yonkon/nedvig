...
onchange='{if $fields.parent_category_c}update_parent_category("");{/if}document.{$form_name}.{{sugarvar key='name'}}.value="";document.{$form_name}.parent_id.value=""; {{sugarvar key='name'}}changeQS(); checkParentType(document.{$form_name}.parent_type.value, document.{$form_name}.btn_{{sugarvar key='name'}});'>
...
{{/if}}
{if $fields.parent_category_c}
<br>
<!-->
<select name='parent_category_c' tabindex="{{$tabindex}}" id='parent_category_c' title='{{$vardef.help}}' >
{html_options options=$fields.parent_category_c.options selected=$fields.parent_category_c.value }
</select>
<script>
var parent_category_array={ldelim}
{foreach from=$fields.parent_category_c.options key=key item=it}
"{$key}":"{$it}",
{/foreach}
{rdelim};
{literal}
function update_parent_category(targetValue){
 key=document.getElementById('parent_type').value;
 targetField = document.getElementById('parent_category_c');
 if(targetField.options.length > 0) {
			for(i = targetField.options.length - 1 ; i >= 0; i--) {
				targetField.remove(i);
			}
		}
	for (i in parent_category_array){
	 if (i==''||i.indexOf(key)>=0){
				selectedOption = (i==targetValue);
				opt = new Option(parent_category_array[i], i, selectedOption, selectedOption);
...
{if $fields.parent_category_c}
<script>
update_parent_category("{$fields.parent_category_c.value}")
</script>
{/if}