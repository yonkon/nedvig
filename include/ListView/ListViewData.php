...
class ListViewData {
	...
	function getListViewData($seed, $where, $offset=-1, $limit = -1, $filter_fields=array(),$params=array(),$id_field = 'id') {
		...
		if( $count >= $limit && $totalCounted){
                                                if(isset($ret_array['select_old'])){
                                                    $main_query = $ret_array['select_old'] . $params['custom_select'] . $ret_array['from'] . $params['custom_from'] . $ret_array['inner_join']. $ret_array['where'] . $params['custom_where'] . $ret_array['order_by'] . $params['custom_order_by'];
                                                }   
			$totalCount  = $this->getTotalCount($main_query);
		}
