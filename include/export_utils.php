	...
	// setup the "header" line with proper delimiters
                 $headers_label = array();
                    foreach ($fields_array as $key =>$value){
                        array_push( $headers_label, isset($focus->field_defs[$value]['vname']) 
                                && isset($mod_strings[$focus->field_defs[$value]['vname']])?
                                $mod_strings[$focus->field_defs[$value]['vname']] : $value );
                    }
                
	$header = implode("\"".getDelimiter()."\"", $headers_label);
	...
	while($val = $db->fetchByAssoc($result, -1, false)) {
		...
		foreach ($vals as $key => $value) {
			...
			// Bug 32463 - Properly have multienum field translated into something useful for the client
                                                    if(isset($focus->field_name_map[$fields_array[$key]]['type']) && $focus->field_name_map[$fields_array[$key]]['type'] == 'enum'){
			    ...
			    if ( isset($focus->field_name_map[$fields_array[$key]]['options'])
			            && isset($app_list_strings[$focus->field_name_map[$fields_array[$key]]['options']]) ) {                                                                                                           
                                                                if ( isset($app_list_strings[$focus->field_name_map[$fields_array[$key]]['options']][$ivalue]) ) {
            ...
            //#3783
            if ($type == 'sphr_Client' && $fields_array[$key] == 'status')
                $value = $app_list_strings['client_status_list'][$value];
            //---
                                           
                                                array_push($new_arr, preg_replace("/\"/","\"\"", preg_replace("/\r\n/","\n",$value)));
		}
